...
export default function DashboardPage({ user, profile }: DashboardPageProps) {
  ...
  const handleGenerateDeck = async () => {
    ...
    if (!profile.companyDescription?.trim() && !profile.pitchSummary?.trim()) {
      alert("Please fill in your Business Description or Elevator Pitch in your Profile tab first.");
    ...
    try {
      ...
      if (slides.length === 0) throw new Error('No slides were generated');
      const newDeckId = `deck-${Date.now()}`;
      await setDoc(doc(db, 'pitchDecks', newDeckId), {
        id: newDeckId,
        userId: user.uid,
        projectName: `${profile.companyName || 'My Startup'} Pitch Deck`,
      ...
      alert('Failed to generate pitch deck: ' + err.message);
    } finally {
  ...
  const bgImageUrl = currentSlideData?.imageKeywords
    ? `https://picsum.photos/seed/${encodeURIComponent(currentSlideData.imageKeywords)}/1200/800`
    : `https://picsum.photos/seed/business/1200/800`;
                ...
                <div className="flex flex-wrap items-center gap-4">
                  {isPremium ? (
                    <button
                      ...
                      className="px-8 py-4 bg-brand-section border border-brand-accent/30 text-brand-accent text-sm font-black uppercase tracking-widest rounded-3xl hover:border-brand-accent/60 hover:scale-105 active:scale-95 transition-all flex items-center gap-3 disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                      {generatingDeck ? <><div className="w-4 h-4 border-2 border-brand-accent border-t-transparent rounded-full animate-spin" /> Generating...</> : <><Wand2 size={16} /> Generate with DLAB</>}
                    </button>
                  ) : (
                    <Link
                      to="/pricing"
                      className="px-8 py-4 bg-brand-section border border-white/10 text-white/40 text-sm font-black uppercase tracking-widest rounded-3xl flex items-center gap-3 cursor-pointer hover:border-brand-accent/30 hover:text-brand-accent/60 transition-all"
                      title="Upgrade to unlock AI pitch deck generation"
                    >
            ...
            <div className="hidden print:block space-y-0 bg-white">
               {profile.pitchDeck.slides.map((slide, sIdx) => {
                  const printBgUrl = slide.imageKeywords
                    ? `https://picsum.photos/seed/${encodeURIComponent(slide.imageKeywords)}/1200/800`
                       ...
                       <div className="relative z-10">
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-16">
                             ...
                             <div className="bg-neutral-50 p-12 rounded-[3.5rem] border-4 border-dashed border-neutral-200 flex flex-col justify-center items-center text-center relative overflow-hidden">
                                <img 
                                   src={`https://picsum.photos/seed/${encodeURIComponent(slide.imageKeywords || 'modern business')}/600/400`}
                     ...
                     <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-24">
                        ...
                        <div className="space-y-12">
                           <div className="bg-neutral-50 p-12 rounded-[4rem] border-4 border-dashed border-neutral-100 flex flex-col justify-center items-center text-center min-h-[450px] group transition-all hover:bg-neutral-100 relative overflow-hidden shadow-inner">
                              <img 
                                src={`https://picsum.photos/seed/${encodeURIComponent(currentSlideData?.imageKeywords || 'business technology')}/800/600`}
